...
group.add_argument("-a", "--add", action = "store_true", help = "add a new guitarist or band")
group.add_argument("-d", "--database", action = "store_true", help = "add a new guitarist or band")
...
if args.database:
    ...
    if args.add:
        ...
        print ("The guitar hero of", db["Bands"].loc[db["Bands"].str.lower()==answer.lower()].values[0], "is", db["Players"].loc[db["Bands"].str.lower()==answer.lower()].values[0])
    elif check.check_guitarist(answer):
        print(db["Players"].loc[db["Players"].str.lower()==answer.lower()].values[0], "is the guitar hero of",db["Bands"].loc[db["Players"].str.lower()==answer.lower()].values[0] )
    else:
